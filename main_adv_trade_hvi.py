...
model_out = './checkpoint/' + opt.data+'_'+opt.model+'_'+str(opt.max_norm)+'_adv_trade_hvi.pth'
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader)):
        ...
        loss_robust = criterion_kl(F.log_softmax(outputs, dim=1), F.softmax(logits, dim=1))
        loss_trade = loss_natural + 10*loss_robust

        loss = loss_trade + beta*kl
        loss.backward()
        optimizer.step()
