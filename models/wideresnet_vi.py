...
class BasicBlock(nn.Module):
    def __init__(self, sigma_0, init_s, in_planes, out_planes, stride):
        ...
        self.init_s = init_s
        self.bn1 = RandBatchNorm2d(self.sigma_0,0,self.init_s,in_planes)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = RandConv2d(self.sigma_0,0,self.init_s,in_planes, out_planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = RandBatchNorm2d(self.sigma_0,0,self.init_s,out_planes)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv2 = RandConv2d(self.sigma_0,0,self.init_s,out_planes, out_planes, kernel_size=3, stride=1,
                               padding=1, bias=False)
        self.equalInOut = (in_planes == out_planes)
        self.convShortcut = (not self.equalInOut) and RandConv2d(self.sigma_0,0,self.init_s,in_planes, out_planes, kernel_size=1, stride=stride,
...
class WideResNet(nn.Module):
    def __init__(self, args, sigma_0, init_s, depth, num_classes, widen_factor=1, mean=0.5, std=0.25):
        ...
        self.conv1 = RandConv2d(self.sigma_0,0,self.init_s, 3, nChannels[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        # 1st block
        ...
        self.bn1 = RandBatchNorm2d(self.sigma_0,0,self.init_s,nChannels[3])
        self.relu = nn.ReLU(inplace=True)
        self.fc = RandLinear(self.sigma_0,0,self.init_s,nChannels[3], num_classes)
